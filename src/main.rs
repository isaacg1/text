...
fn editor_draw_status_bar(editor_config: &EditorConfig, append_buffer: &mut String) {
    ...
    name.truncate(20);
    let dirty = if editor_config.modified { "(modified)" } else { "" };
    ...
    let mut right_status = format!("{}/{}", editor_config.cursor_y + 1, editor_config.rows.len());
...
fn editor_refresh_screen(editor_config: &mut EditorConfig) {
    ...
    let cursor_control = format!("\x1b[{};{}H",
                                 ...
                                 (editor_config.cursor_x - editor_config.col_offset) + 1);
...
fn editor_move_cursor(editor_config: &mut EditorConfig, key: EditorKey) {
    ...
    match key {
        EditorKey::ArrowUp => if editor_config.cursor_y > 0 {
            editor_config.cursor_y -= 1
        },
        EditorKey::ArrowDown => if editor_config.cursor_y < editor_config.rows.len() {
            editor_config.cursor_y += 1
        },
        EditorKey::ArrowLeft => if editor_config.cursor_x > 0 {
            editor_config.cursor_x -= 1
        ...
        EditorKey::ArrowRight => if editor_config.cursor_x < row_len {
            editor_config.cursor_x += 1
        } else if editor_config.cursor_x == row_len && editor_config.cursor_y < editor_config.rows.len() {
            editor_config.cursor_y += 1;
            editor_config.cursor_x = 0
        },
        EditorKey::PageUp => {
        ...
        EditorKey::PageDown => {
            editor_config.cursor_y = editor_config.row_offset + editor_config.screen_rows - 1;
            for _ in 0..editor_config.screen_rows {
    ...
    if c == EditorKey::Verbatim(ctrl_key('q')) {
        if editor_config.modified && editor_config.quit_times > 0 {
            ...
            editor_set_status_message(
                editor_config,
                &format!("Warning: File has unsaved changes. Press Ctrl-Q {} more times to quit.",
                       quit_times));
        ...
        match c {
            ...
            EditorKey::Verbatim(chr) if chr == '\r'
            	=> editor_insert_newline(editor_config),
            ...
            EditorKey::Verbatim(chr) if chr == ctrl_key('s')
            	=> match editor_save(editor_config) {
                	Ok(len) => editor_set_status_message(editor_config,
                	                                 &mut format!("{} bytes written to disk", len)),
                	Err(e) => editor_set_status_message(editor_config,
                	                                 &mut format!("Saving failed with {}", e)),
            	},
            EditorKey::Verbatim(chr)
                => editor_insert_char(editor_config, chr),
        };
        editor_config.quit_times = 3;
        true
    }
...
fn main() {
    ...
    editor_set_status_message(&mut editor_config, "Help: CTRL-q to exit.");
    loop {
